...
| URL | 役割 | 認証 |
|-----|------|------|
| `/` | LP（デザイン2・レトロ×モダン）※メイン | 不要（ログイン済みなら /v2/dashboard へ） |
| `/v1` | LP（デザイン1・旧デザイン） | 不要 |
| `/v2` | `/` へリダイレクト | - |
| `/sign-in` | ログイン | 不要 |
| `/sign-up` | サインアップ | 不要 |
| `/onboarding` | 初回登録（氏名・居住地・学校・部活・卒業年） | 要 |
| `/dashboard` | `/v2/dashboard` へリダイレクト | - |
| `/v2/dashboard` | ダッシュボード（学校掲示板・帰省中・部室）※メイン | 要 |
...
| グループ | ✅ | 作成・参加・招待リンク、投稿 |
| イベント | ✅ | 作成・参加・キャンセル・削除 |
| イベント相談フロー | ✅ | 掲示板で相談→コメントで議論→話がまとまったらイベント作成 |
...
| 確認したいこと | アクセス先 |
|----------------|------------|
| LP（メイン） | http://localhost:3000 |
| LP（旧デザイン） | http://localhost:3000/v1 |
| ログイン | http://localhost:3000/sign-in |
| サインアップ | http://localhost:3000/sign-up |
| ダッシュボード | ログイン後 http://localhost:3000/v2/dashboard |
...
| `FINAL_ASSIGNMENT_CHECKLIST.md` | 最終課題提出物チェックリスト |
| `EVENT_CONSULTATION_FLOW_案A.md` | イベント相談フロー（案A）の説明 |
